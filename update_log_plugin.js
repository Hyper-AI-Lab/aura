...
module.exports = {
  ...
  register: (api) => {
    ...
    const logToFile = (msg) => {
        try {
            fs.appendFileSync("/tmp/rmp_plugin.log", new Date().toISOString() + " - " + msg + "\\n");
        } catch(e) {}
    };

    api.on("message_received", (event) => {
        logToFile("message_received: " + JSON.stringify(event));
    });

    api.on("before_message_write", (event) => {
       logToFile("before_message_write: " + JSON.stringify(event));
       ...
       try {
          ...
          logToFile("Sending to RMP: " + payload);
          ...
          logToFile("RMP Response: " + JSON.stringify(data));
          ...
          logToFile("Failed to auto-route message: " + err.message);
