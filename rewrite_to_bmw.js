...
const newContent = `const { execSync } = require('child_process');
  ...
  register: (api) => {
    ...
    api.on("before_message_write", (event) => {
       ...
       try {
          ...
          const out = execSync("curl -s -X POST http://127.0.0.1:8000/tasks -H 'Content-Type: application/json' -d '" + payload + "'");
